...
var Coin = function(X,Y) {
	...
	this.makeGraphic(8,8,-256);
	this.velocity.set_x(Platform.SCROLL_VELOCITY);
};
$hxClasses["Coin"] = Coin;
Coin.__name__ = ["Coin"];
...
var DustEmitter = function(X,Y,Size) {
	if(Size == null) Size = 100;
	if(Y == null) Y = 0;
	if(X == null) X = 0;
	flixel.effects.particles.FlxEmitter.call(this,X,Y,Size);
	this.setXSpeed(Platform.SCROLL_VELOCITY - 10,Platform.SCROLL_VELOCITY + 10);
	this.setYSpeed(-20,-120);
	this.set_width(16);
	this.set_gravity(800);
	var _g1 = 0;
	var _g = Math.floor(Size / 2);
	while(_g1 < _g) {
		var i = _g1++;
		this._particle = new flixel.effects.particles.FlxParticle();
		this._particle.makeGraphic(2,2,-1);
		this._particle.set_visible(false);
		this.add(this._particle);
		this._particle = new flixel.effects.particles.FlxParticle();
		this._particle.makeGraphic(1,1,-1);
		this._particle.set_visible(false);
		this.add(this._particle);
	}
};
$hxClasses["DustEmitter"] = DustEmitter;
DustEmitter.__name__ = ["DustEmitter"];
DustEmitter.__super__ = flixel.effects.particles.FlxEmitter;
DustEmitter.prototype = $extend(flixel.effects.particles.FlxEmitter.prototype,{
	_particle: null
	,__class__: DustEmitter
});
...
var HUD = function() {
	...
	this._sprBack = new flixel.FlxSprite().makeGraphic(flixel.FlxG.width,20,-16777216);
	flixel.util.FlxSpriteUtil.drawRect(this._sprBack,0,19,flixel.FlxG.width,1,-1);
	this._txtScore = new flixel.text.FlxText(0,2,0,"0",8);
	this._txtScore.setBorderStyle(1,-8355712,1,1);
	this._txtScore.set_alignment("right");
...
HUD.prototype = $extend(flixel.group.FlxTypedGroup.prototype,{
	...
	,_txtScore: null
	,update: function() {
		this._txtScore.set_text(Std.string(Reg.score));
		this._txtScore.set_x(flixel.FlxG.width - 30);
		flixel.group.FlxTypedGroup.prototype.update.call(this);
	}
	,__class__: HUD
});
...
var Platform = function(x,y) {
	if(x == null) x = 0;
	if(y == null) y = Platform.DEFAULT_Y;
	flixel.FlxSprite.call(this,x,y);
	this.makeGraphic(128,512,-1);
	this.set_solid(true);
...
Platform.__super__ = flixel.FlxSprite;
Platform.prototype = $extend(flixel.FlxSprite.prototype,{
	update: function() {
		if(this.x < -256) this.set_exists(false);
		flixel.FlxSprite.prototype.update.call(this);
	}
	,__class__: Platform
});
var PlatformGroup = function(X,Y) {
	if(Y == null) Y = 0;
	if(X == null) X = 0;
	flixel.group.FlxTypedGroup.call(this);
	this.set_maxSize(20);
	...
	this.add(platform);
	this.addNextPlatform(true);
};
$hxClasses["PlatformGroup"] = PlatformGroup;
PlatformGroup.__name__ = ["PlatformGroup"];
PlatformGroup.__super__ = flixel.group.FlxTypedGroup;
PlatformGroup.prototype = $extend(flixel.group.FlxTypedGroup.prototype,{
	update: function() {
		if(this.getLastPlatform().x < 700) this.addNextPlatform();
		flixel.group.FlxTypedGroup.prototype.update.call(this);
	}
	,getLastPlatform: function() {
		return this.members[this.length - 1];
	}
	,makeOrRecyclePlatform: function() {
		var platform = this.getFirstAvailable(Platform,true);
		if(platform != null) {
			this.remove(platform,true);
			platform.set_exists(true);
		} else platform = new Platform();
		return platform;
	}
	,addNextPlatform: function(useDefaults) {
		if(useDefaults == null) useDefaults = false;
		var lastPlatform = this.getLastPlatform();
		var nextPlatform = this.makeOrRecyclePlatform();
		nextPlatform.set_y(Platform.DEFAULT_Y);
		nextPlatform.set_x(lastPlatform.x + 128 - 1);
		if(!useDefaults) {
			if((flixel.util.FlxRandom._internalSeed = flixel.util.FlxRandom._internalSeed * 48271 % 2147483647 & 2147483647) / 2147483647 < 0.25) {
				var _g = nextPlatform;
				_g.set_y(_g.y - 64);
			}
			if((flixel.util.FlxRandom._internalSeed = flixel.util.FlxRandom._internalSeed * 48271 % 2147483647 & 2147483647) / 2147483647 < 0.1) {
				var _g1 = nextPlatform;
				_g1.set_x(_g1.x + 128);
			}
		}
		this.add(nextPlatform);
	}
	,__class__: PlatformGroup
});
...
PlayState.prototype = $extend(flixel.FlxState.prototype,{
	...
	,create: function() {
		...
		this._dustEmitter = new DustEmitter();
		this._player = new Player(80,200);
		this._player.dustEmitter = this._dustEmitter;
		this.add(this._player);
		this.add(this._dustEmitter);
		this._coinGroup = new flixel.group.FlxTypedGroup();
		this.add(this._coinGroup);
		var coin = new Coin(600,120);
		this._coinGroup.add(coin);
		this._platformGroup = new PlatformGroup();
		this.add(this._platformGroup);
		this._hud = new HUD();
		this.add(this._hud);
		flixel.FlxState.prototype.create.call(this);
	}
	...
	,update: function() {
		flixel.FlxState.prototype.update.call(this);
		flixel.FlxG.overlap(this._player,this._platformGroup,null,flixel.FlxObject.separate);
		flixel.FlxG.overlap(this._player,this._coinGroup,($_=this._player,$bind($_,$_.touchCoin)));
	}
	,__class__: PlayState
});
var Player = function(X,Y) {
	...
	this.initX = X;
	this.initY = Y;
	this.lastVelocity = new flixel.util.FlxPoint(0,0);
...
Player.prototype = $extend(flixel.FlxSprite.prototype,{
	...
	,update: function() {
		...
		this.movement();
		if(this.alive) {
			if(this.shouldBeDead()) this.kill(); else Reg.score += 1;
		}
		if(this.outOfBounds()) this.reset(this.initX,this.initY);
		flixel.FlxSprite.prototype.update.call(this);
	}
	,kill: function() {
		this.set_alive(false);
	...
	,reset: function(X,Y) {
		...
		this.lastVelocity.copyFrom(this.velocity);
	}
	,touchCoin: function(P,C) {
		if(P.alive && P.exists && C.alive && C.exists) {
			Reg.score += 1000;
			C.kill();
		}
	}
	,shouldBeDead: function() {
		return this.outOfBounds() || (this.touching & 16) > 0;
	}
	,movement: function() {
		var _up = flixel.FlxG.keys.checkKeyStatus(["UP","W","SPACE"],1);
		var touched = flixel.FlxG.touches.getFirst();
		_up = _up || touched != null;
		if(_up && (this.touching & 4096) > 0) this.velocity.set_y(this.JUMP_SPEED);
		this.lastVelocity.copyFrom(this.velocity);
	}
	...
	,animateCollision: function() {
		var shakeY = Math.abs(this.lastVelocity.y - this.velocity.y) / 30000;
		var shakeAmount = shakeY + Math.abs(this.lastVelocity.x - this.velocity.x) / 30000;
		if(shakeAmount > 0.01 && ((this.touching & 4369) > 0 && (this.wasTouching & 4369) <= 0)) flixel.FlxG.camera.shake(shakeAmount,0.1);
		if((this.touching & 4096) > 0 && (this.wasTouching & 4096) <= 0 && this.dustEmitter != null) {
			this.dustEmitter.set_x(this.x + 8);
			this.dustEmitter.set_y(this.y + 15);
			var numParticles = 8 + Math.floor(Math.min(30,shakeY * 1000));
			var period = 0.08;
			var frequency = period / numParticles;
			this.dustEmitter.start(false,1.5,frequency,numParticles);
		}
	}
	,__class__: Player
});
...
flixel.system._FlxPreloader.GraphicLogoLight.preload = function() {
	var o = document.createElement("img");
	ApplicationMain.loadEmbed(flixel.system._FlxPreloader.GraphicLogoLight.image = o);
...
flixel.system._FlxPreloader.GraphicLogoCorners.preload = function() {
	var o = document.createElement("img");
	ApplicationMain.loadEmbed(flixel.system._FlxPreloader.GraphicLogoCorners.image = o);
...
ApplicationMain.config = { antialiasing : 0, background : 0, borderless : false, depthBuffer : false, fps : 60, fullscreen : false, height : 576, orientation : "portrait", resizable : true, stencilBuffer : true, title : "FlixelRunner", vsync : true, width : 1024};
...
Platform.SCROLL_VELOCITY = -300;
Platform.DEFAULT_X = 80;
Platform.DEFAULT_Y = 216;
Reg.levels = [];
Reg.level = 0;
Reg.scores = [];
Reg.score = 0;
